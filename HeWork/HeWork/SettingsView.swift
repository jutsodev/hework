                        ...
                        Button(action: { showProfile = true }) {
                            HStack(spacing: 14) {
                                ...
                                VStack(alignment: .leading, spacing: 4) {
                                    Text(authViewModel.currentUser?.username ?? "Пользователь").font(.system(size: 18, weight: .semibold)).foregroundColor(.white)
...
struct SettingsSection<Content: View>: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            Text(header).font(.system(size: 12, weight: .semibold)).foregroundColor(.appTextSecondary)
                .padding(.horizontal, 20).padding(.top, 24).padding(.bottom, 8)
            VStack(spacing: 0) { content() }.background(Color.appCard).cornerRadius(14).padding(.horizontal, 16)
        }
    }
}
