...
struct ProfileView: View {
    ...
    var body: some View {
        NavigationStack {
                    ...
                    VStack(spacing: 20) {
                        ...
                        VStack(spacing: 4) {
                            ...
                            HStack(spacing: 6) {
                                ...
                                Text("ОНЛАЙН").font(.system(size: 11, weight: .semibold)).foregroundColor(.appGreen)
                            }.padding(.top, 4)
                        }
            ...
            .toolbar { ToolbarItem(placement: .cancellationAction) { Button("Закрыть") { dismiss() }.foregroundColor(.appAccent) } }
        }
    }
}
...
struct ProfileInfoRow: View {
    ...
    var body: some View {
        HStack(spacing: 12) {
            Image(systemName: icon).foregroundColor(.appAccent).frame(width: 24)
            ...
            Button(action: { UIPasteboard.general.string = value; showCopied = true; DispatchQueue.main.asyncAfter(deadline: .now() + 1.5) { showCopied = false } }) {
                Image(systemName: showCopied ? "checkmark" : "doc.on.doc").font(.system(size: 14)).foregroundColor(.appAccent)
            }
        }
